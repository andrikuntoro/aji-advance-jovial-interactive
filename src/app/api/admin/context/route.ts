...
export async function GET() {
  ...
  if (!context) {
    return NextResponse.json({ error: "Custom context file not found or corrupted" }, { status: 404 });
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body || !body.persona || !body.scenario || !body.objectionFramework) {
      return NextResponse.json({ error: "Invalid context structure provided" }, { status: 400 });
    }

    const success = writeContextSafely(body);
    if (!success) {
      return NextResponse.json({ error: "Failed to write context file to disk" }, { status: 500 });
    }

    return NextResponse.json({ success: true, message: "AI Knowledge context updated successfully!" });
  } catch (error) {
